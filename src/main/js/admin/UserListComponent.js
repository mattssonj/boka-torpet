...
export default function UserListComponent(showAll) {
    ...
    const errorMessage = showAll ? 'Kunde inte hämta alla användare. Error ' : 'Kunde inte hämta dina användare. Error '
    const forbiddenMessage = 'Bara admins kan använda denna vy. Kontakta den som skapade ditt konto för frågor.'
    ...
    const tabContent = (user) => {
        let eventKey = '#' + user.username
        return (<Tab.Pane eventKey={eventKey}>
    ...
    return (
        <Col>
            <Row>
                <Col>
                    <Button variant="outline-secondary" onClick={updateUsers}>Uppdatera listan</Button>
                </Col>
            </Row>
            <br/>
            <Row>
                                ...
                                <ListGroup>
                                    {users.map(user => (
                                        <ListGroup.Item action
                                                        href={'#' + user.username}>{user.username}</ListGroup.Item>))}
                                </ListGroup>
                            </Col>
                            <Col sm={8}>
                                <Tab.Content>
