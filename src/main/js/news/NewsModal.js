...
export default function NewsModal({show, hideFunction, newMessage}) {
    ...
    const postRequest = async () => {
        return Axios.post('/api/messages', formValues)
    }

    return (
        <Modal show={show} onHide={hideFunction}>
            <Modal.Header><Modal.Title>Skriv nytt meddelande</Modal.Title></Modal.Header>
            <Modal.Body>
                <Form>
            ...
            <Modal.Footer>
                ...
                <Button variant="primary" onClick={!isCreating ? createNews : null}
                        disabled={isCreating}>{isCreating ? 'Uppdaterar...' : 'Uppdatera'}</Button>
            </Modal.Footer>
        </Modal>
    );
}