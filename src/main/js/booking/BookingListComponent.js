...
export default function Bookings() {
    ...
    const showLoggedInBookings = (newBookings) => {
        const bookingsToShow = newBookings ? newBookings : bookings
        const bookerFilter = (bookings) => bookings.filter(booking => booking.booker === loggedInUsername)
        setShowing({ongoing: bookerFilter(bookingsToShow.ongoing), upcoming: bookerFilter(bookingsToShow.upcoming)})
        setShowAll(false)
    }
    ...
    return (
        <Col>
            ...
            <Row className="justify-content-md-center">
                ...
                <Col md="auto"><Button variant={!showAll ? "secondary" : "outline-secondary"} onClick={() => showLoggedInBookings(null)}>Visa
                    mina</Button></Col>
                <Col md="auto"><Button variant={showAll ? "secondary" : "outline-secondary"} onClick={() => showAllBookings(null)}>Visa
                    alla</Button></Col>
            </Row>
            <Row><Col><h1>På torpet just nu</h1></Col></Row>
            <Row>
                <Col>
                    {mapListItems(showing.ongoing, 'Ingen verkar vara där just nu')}
                </Col>
            </Row>
            <Row><Col><h1>Kommande</h1></Col></Row>
            <Row>
                <Col>
