...
export default function CreateBooking() {
    ...
    const [showAlert, setShowAlert] = useState({show: false, error: false, message: {}})

    const hideModal = () => setShowModal(false)
    const hideAlert = () => setShowAlert({show: false, error:false, message: {}})
    const presentAlert = (newMessage, error) => {
        setShowAlert({show: true, error: error, message: newMessage})
        setShowModal(false)
    }

    return (
        <Col>
            <Alert show={showAlert.show} variant={showAlert.error ? "danger": "success"}>
